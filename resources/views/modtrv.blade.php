...
<div class="main-banner" id="top">
    ...
    <div class="video-overlay header-text">
        <div class="caption">
            <br>
            <form action="{{url('misetrv/' .$travaille->id)}}" id="ft-form" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
                @csrf
                <!--<fieldset>
                  <legend>Job</legend>
                  <label>
                ...
                <fieldset>
                    <br><br>
                  <h5 class="btn btn-primary">travaille description</h5>
                  <div class="two-cols">
                    <h6>
                        l'intitulé du travaille *
                      <input type="text" value="{{ $travaille->intitule}}" style=" background-color: #d1d1d1; color:#000000" name="intitule" required>
                    </h6>
                    <h6>
                      le salaire *  DZD
                      <input type="number" placeholder="DZD" value="{{ $travaille->salaire}}" style=" height: 42px; background-color: #d1d1d1; color:#000000" name="salaire" required>
                    </h6>
                  </div>
                  <div class="two-cols">
                    <h6>
                      les horaires du travaille * (heures)
                      <input class="" type="number" placeholder="HEURES" value="{{ $travaille->horaires}}" style=" height: 42px; background-color: #d1d1d1; color:#000000" name="horaires" required>
                    </h6>
                    <h6>
                      la wilaya *
                      <select name="ville" style="background-color: #d1d1d1; color:#000000" required>
                        <option value="{{ $travaille->nom_wilaya}}">{{$travaille->nom_wilaya}} </option>
                        @foreach ($wilaya as $item)
                        @if ($item->nom!=$travaille->nom_wilaya)
                  ...
                  <div class="two-cols" >
                  <h6>
                    la localisation du travaille *
                    <input type="text" value="{{ $travaille->localisation}}" style=" background-color: #d1d1d1; color:#000000" name="localisation" required>
                  </h6>
                  <h6>
                    Informations générales sur le travaille
                    <input type="text" value="{{ $travaille->information}}" style=" background-color: #d1d1d1; color:#000000" name="information">
                  </h6>
                </div>
                  <!--<div class="two-cols">
                    <h6>
                  ...
                  <div class="two-cols">
                    <h6>
                      diplome/formation nécessaire
                      <input type="text" value="{{ $travaille->diplome}}" style=" background-color: #d1d1d1; color:#000000" name="diplome" >
                    </h6>
                    <h6>
                        autres nécessité de travaille
                      <input type="text" value="{{ $travaille->necessite}}" style=" background-color: #d1d1d1;  color:#000000" name="necessite">
                    </h6>
                  </div>
                  <div class="two-cols">
                    <h6>
                        photo du lieu de travail (photo descriptive)
                      <!-- <p>Possible file types: DOC, PDF. Maximum file size: 10 MB.</p> -->
                      <input type="file"  style=" background-color: #d1d1d1; color: #000000" name="photo" accept=".png, .jpg, .jpeg" >
                    </h6>
                    <h6>
                        contrat(tout information doit être montioner)
                      <!-- <p>Possible file types: DOC, PDF. Maximum file size: 10 MB.</p> -->
                      <input type="file"  style=" background-color: #d1d1d1; color: #000000" name="contrat" accept=".pdf" >
                    </h6>
                    </div>
                    <input type="hidden" value="{{ Auth::user()->id}}" name="user_id" required>
                        <input type="hidden" value="{{ Auth::user()->name}}" name="user_name" required>
                    <div> <a  href="{{ url('vtrv') }}" class="btn btn-primary col-md-4 " style="float: left ; color:white">retour</a>
