...
<div class="card">
    <div class="card-header">
        <h4 style="color: white">liste des cvs</h4>
    </div>
    <div class="card-body">
            ...
            <tbody>
                @foreach ( $cv as $item)
                 ...
                 <tr style="background-color: rgb(34, 34, 34)">
                     ...
                     <td>
                         <embed src="{{ asset('images/cv/' .$item->cv)}}" class="cate-image" alt="file here" width="100%">
                     </td>
                      <td>
                        <form action="{{url('approuver/'.  $item->id )}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                          ...
                          <br>
                          <a href="{{url('suprimer_cv/' .$item->id)}}" class="btn btn-danger">suprimer</button>
                      </td>
                 </tr>
                 @endif
                 @if ($item->approuvé==1)
